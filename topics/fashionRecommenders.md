...
Hyper-personalization is seen both in an algorithmic and human-centric approach. Zipfit Denim in Chicago collects metrics from its inventory of denim and takes in explicit user preferences to suggest individual pairs of jeans that will fit the user, meaning men walk in and out of the store in minutes with well-fitting jeans. This goes deeper than brand-level fit recommendations. Human-validated decisions also permeate the fashion recommender space. PS Dept in NYC started out retrofitting brick-and-mortar retailers with an iPhone app that connects existing store associates to consumer requests anywhere, so their recommendations are all individually tailored for active users. Subscriptions services like Stitch Fix partner algorithms with "curators" for users, who can further tailor recommendations to the user before sending out a box of 5 items at a time. To get back in touch with the human side of fashion, the recommender literature would benefit from a field study of user feedback on various systems, beyond metrics like click-through rates.  
