...
#read in file, time series has data for all 365 days, for weekends or holidays we use the previous day's close
#need 365 days for fourier transform (periodic seasonality) since 365 is hardcoded

df = pd.read_csv('/Users/shioulinsam/Documents/Data/SPSectorFilled.csv')

#Dataset consists of the following ETFs: XLY, XLP, XLE, XLF, XLV, XLI, XLB, XLK, XLU
data_string = 'XLY'

df['ds'] = df['Date']
df['y'] = df[data_string]

#set changepoint parameter, default is 0.05. When set to a large number (eg 0.20)
#the system is more flexible (ie, tend to overfit)
...
#fill_between does not work
#ax.fill_between(forecast['ds'], forecast['yhat_lower'], forecast['yhat_upper'], c=forecast_color, alpha=0.2)
ax.plot(forecast['ds'], forecast['yhat_lower'], ls='--', c=forecast_color, alpha=0.5)
ax.plot(forecast['ds'], forecast['yhat_upper'], ls='--', c=forecast_color, alpha=0.5)

ax.grid(True, which='major', c='gray', ls='-', lw=1, alpha=0.2)
ax.set_xlabel('ds')
ax.set_ylabel('y')
fig1.show()
fig_string = '/Users/shioulinsam/Documents/Figures/'+data_string + 'forecast.png'
fig1.tight_layout()
...
#ax.fill_between(forecast['ds'].values,forecast['trend_lower'], forecast['trend_upper'], c=forecast_color)
ax.plot(forecast['ds'], forecast['trend_lower'], ls='--', c=forecast_color, alpha=0.5)
ax.plot(forecast['ds'], forecast['trend_upper'], ls='--', c=forecast_color, alpha=0.5)
ax.set_xlabel('ds')
ax.set_ylabel('trend')
...
fig_string = '/Users/shioulinsam/Documents/Figures/'+data_string + 'component.png'
fig2.tight_layout()
